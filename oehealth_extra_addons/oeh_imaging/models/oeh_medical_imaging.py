...
# [GCC 8.4.0]
# Embedded file name: D:\DEV\Workspaces\odoo10\addons_custom\oehealth_extra_addons\oeh_imaging\models\oeh_medical_imaging.py
...
class OeHealthImagingTestType(models.Model):
    ...
    _sql_constraints = [
     ('name_uniq', 'unique(name)', 'The Imaging test type name must be unique')]
...
class OeHealthImagingTypeManagement(models.Model):
    ...
    def action_imaging_invoice_create(self):
        ...
        for imaging in self:
            if imaging.patient:
                curr_invoice = {'partner_id': imaging.patient.partner_id.id, 'account_id': imaging.patient.partner_id.property_account_receivable_id.id, 
                ...
                if inv_ids:
                    ...
                    if imaging.test_type:
                        curr_invoice_line = {'name': 'Charge for ' + str(imaging.test_type.name) + ' Imaging test', 
        ...
        return {'domain': "[('id','=', " + str(inv_id) + ')]', 'name': 'Imaging Test Invoice', 
           'view_type': 'form', 
           'view_mode': 'tree,form', 
           'res_model': 'account.invoice', 
           'type': 'ir.actions.act_window'}
# okay decompiling oeh_medical_imaging.pyc
