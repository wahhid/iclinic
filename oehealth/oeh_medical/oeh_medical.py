...
# [GCC 8.4.0]
# Embedded file name: D:\Workspaces\Odoo10\mod\Health\addons-comcustom\oehealth\oeh_medical\oeh_medical.py
# Compiled at: 2018-06-13 06:57:39
import datetime
        ...
        def compute_age_from_dates(patient_dob, patient_deceased, patient_dod):
            ...
            if patient_dob:
                ...
                if patient_deceased:
                    ...
                    delta = dod - dob
                    deceased = ' (deceased)'
                    years_months_days = str(delta.days // 365) + ' years ' + str(delta.days % 365 / 30) + ' months ' + str(delta.days % 365 % 30) + ' days' + deceased
                else:
                    delta = now - dob
                    years_months_days = str(delta.days // 365) + ' years ' + str(delta.days % 365 / 30) + ' months ' + str(delta.days % 365 % 30) + ' days'
            else:
...
class OeHealthPhysician(models.Model):
    ...
    oeh_user_id = fields.Many2one('res.users', string='Responsible ERP User')
    _sql_constraints = [
...
class OeHealthPrescriptions(models.Model):
    ...
    def action_prescription_invoice_create(self):
        ...
        for pres in self:
            if pres.patient:
                curr_invoice = {'partner_id': pres.patient.partner_id.id, 'account_id': pres.patient.partner_id.property_account_receivable_id.id, 
                ...
                if inv_ids:
                    ...
                    if pres.prescription_line:
                        for ps in pres.prescription_line:
                            curr_invoice_line = {'name': ps.name.product_id.name, 
        ...
        return {'domain': "[('id','=', " + str(inv_id) + ')]', 'name': 'Prescription Invoice', 
           'view_type': 'form', 
           'view_mode': 'tree,form', 
           'res_model': 'account.invoice', 
           'type': 'ir.actions.act_window'}
        ...
        for pres in self:
            if not pres.pharmacy:
                ...
                if phy_ids:
                    if pres.prescription_line:
                        for ps in pres.prescription_line:
                            curr_pres_line = {'name': ps.name.id, 
