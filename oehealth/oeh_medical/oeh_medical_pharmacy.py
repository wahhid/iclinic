...
# [GCC 8.4.0]
# Embedded file name: D:\DEV\Workspaces\odoo10\addons_custom\oehealth\oeh_medical\oeh_medical_pharmacy.py
...
class OeHealthPharmacyLines(models.Model):
    ...
    def onchange_prescription_id(self, prescription_id=False):
        ...
        prescription_ids = []
        res = {'value': {'prescription_lines': [], 'doctor': '', 
                     'patient': ''}}
        if not prescription_id:
            return res
        pr = pres_obj.browse(prescription_id)
        res['value'].update({'patient': pr.patient.id, 
           'doctor': pr.doctor.id})
        ...
        if vals:
            for va in vals:
                ...
                if vals1:
                    med_price = vals1[0]
                pres = {'name': va[0], 'indication': va[1], 
                   'qty': va[2], 
                   'actual_qty': va[2], 
            ...
            res['value'].update({'prescription_lines': prescription_ids})
        return res
