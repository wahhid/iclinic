...
# [GCC 8.4.0]
# Embedded file name: D:\DEV\Workspaces\odoo10\addons_custom\oehealth\oeh_lab\oeh_medical_lab.py
...
class OeHealthLabTestUnits(models.Model):
    ...
    _sql_constraints = [
     ('name_uniq', 'unique(name)', 'The Lab unit name must be unique')]
...
class OeHealthLabTests(models.Model):
    ...
    def onchange_test_type_id(self, test_type=False):
        ...
        res['value'].update({'lab_test_criteria': labtest_ids})
        return res
    ...
    def action_lab_invoice_create(self):
        ...
        for lab in self:
            if lab.patient:
                curr_invoice = {'partner_id': lab.patient.partner_id.id, 'account_id': lab.patient.partner_id.property_account_receivable_id.id, 
                   'state': 'draft', 
                   'type': 'out_invoice', 
                ...
                if inv_ids:
                    ...
                    if lab.test_type:
                        curr_invoice_line = {'name': 'Charge for ' + str(lab.test_type.name) + ' laboratory test', 
        ...
        return {'domain': "[('id','=', " + str(inv_id) + ')]', 'name': 'Lab Test Invoice', 
           'view_type': 'form', 
           'view_mode': 'tree,form', 
